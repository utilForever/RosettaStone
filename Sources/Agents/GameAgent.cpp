...
{
	GameAgent::GameAgent(User& user1, User& user2) :
			m_current(user1), m_opponent(user2)
	{
		// Do Nothing
	}

	GameAgent::GameAgent(User&& user1, User&& user2) :
			m_current(std::move(user1)), m_opponent(std::move(user2))
	{
		// Do Nothing
	}
	...
	{
	    m_taskAgent.Notify(std::move(data));
	}
