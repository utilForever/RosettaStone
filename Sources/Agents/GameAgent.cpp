...
void GameAgent::BeginMulligan(User& user)
{
    ...
    if (index[0] >= NUM_BEGIN_DRAW)
    {
        ...
        throw std::runtime_error(
            "Mulligan index should be under NUM_BEGIN_DRAW.");
    }
    ...
    DrawStructure data2(drawType, user.id, static_cast<BYTE>(read),
                        &*hand.end() - read);
    WriteOutputBuffer(reinterpret_cast<BYTE*>(&data2),
                      sizeof(DrawStructure));  // send new card data
}
...
unsigned int GameAgent::MainMenu(User& user, User& enemy)
{
    ...
    GameBrief brief(user.id, enemy.id, user.existMana, enemy.existMana,
                    static_cast<BYTE>(user.hand.size()),
                    static_cast<BYTE>(enemy.hand.size()),
                    static_cast<BYTE>(user.field.size()),
                    static_cast<BYTE>(enemy.field.size()), user.hero,
                    enemy.hero, user.field.data(), user.hand.data(),
                    enemy.field.data());
    ...
    else
    {
        ...
        throw std::runtime_error(
            "Main menu should be in range [0, GAME_MAIN_MENU_SIZE).");
    }
...
void GameAgent::MainUseCard(User& user)
{
    MainUseCardStructure data(
        user.id, user.existMana, static_cast<BYTE>(user.field.size()),
        static_cast<BYTE>(user.hand.size()), user.hand.data());
    WriteOutputBuffer(reinterpret_cast<BYTE*>(&data),
                      sizeof(MainUseCardStructure));
    ...
    {
        MainUseMinionStructure* minion =
            reinterpret_cast<MainUseMinionStructure*>(m_buffer);
        if (user.hand[minion->cardIndex]->GetCost() > user.existMana)
        {
            // requested mana of minion must be under or equal with user's exist
            // mana
            throw std::runtime_error(
                "Cost of the card must be under or equal with user.existMana.");
        }
        ...
        if (minion->position < 0 || minion->position > user.field.size())
        {
            ...
            throw std::runtime_error(
                "Minion position should be in range [0, user.field.size].");
        }
        ...
        if (minion->cardIndex < 0 || minion->cardIndex >= user.hand.size())
        {
            ...
            throw std::runtime_error(
                "Card index should be in range [0, user.hand.size).");
        }
        ...
        user.field.insert(user.field.begin() + minion->position,
                          user.hand[minion->cardIndex]);
...
void GameAgent::MainCombat(User& user)
{
    ...
    MainCombatStructure data(user.id, static_cast<BYTE>(user.field.size()),
    ...
    WriteOutputBuffer(reinterpret_cast<BYTE*>(&data),
                      sizeof(MainCombatStructure));

    // read combating minion and target minion
    ReadInputBuffer(m_buffer, sizeof(TargetingStructure));
    TargetingStructure* targeting =
        reinterpret_cast<TargetingStructure*>(m_buffer);
    ...
    if (src >= user.field.size())
    {
        ...
        throw std::runtime_error(
            "Combat source index must be in range [0, user.field.size).");
    }

    std::vector<Card*>& attacked = user.attacked;
    if (std::find(attacked.begin(), attacked.end(), user.field[src]) !=
        attacked.end())
    {
    ...
    if (dst > opponent.field.size())
    {
        ...
        throw std::runtime_error(
            "Combat target index must be in range [0, opponent.field.size].");
    }
    ...
    {
        int targetHurted = static_cast<int>(target->GetHealth()) -
                           static_cast<int>(source->GetAttack());
        ...
        if (targetHurted > 0)
        {
            ...
            WriteOutputBuffer(reinterpret_cast<BYTE*>(&modified),
                              sizeof(ModifyHealthStructure));
        }
        ...
        else if (dst != 0)
        {
            ...
            WriteOutputBuffer(reinterpret_cast<BYTE*>(&exhausted),
                              sizeof(ExhaustMinionStructure));
        }
    }
    ...
    {
        int sourceHurted = static_cast<int>(source->GetHealth()) -
                           static_cast<int>(target->GetAttack());
...
{
    size_t healthCurrent = m_userCurrent.hero->GetHealth();
    size_t healthOpponent = m_userOpponent.hero->GetHealth();
...
unsigned int GameAgent::Draw(User& user, size_t num)
{
    ...
    if (deck.size() < size)
    {
        ...
        for (size_t i = 1; i <= rest; ++i)
        {
            ...
            WriteOutputBuffer(reinterpret_cast<BYTE*>(&health),
                              sizeof(ModifyHealthStructure));
        }
...
void GameAgent::ModifyMana(User& user, NumericModification mod,
                           unsigned int type, BYTE num)
{
