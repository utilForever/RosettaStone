...
TEST(HunterExpert1Test, EX1_534_SavannahHighmane)
{
    ...
    game.Process(curPlayer, PlayCardTask::Minion(right));
    ...
    game.Process(opPlayer, PlayCardTask::Minion(card2));
    game.Process(opPlayer, AttackTask(card2, center));
    EXPECT_TRUE(center->isDestroyed);
